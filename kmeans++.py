...
def read_data(filename):
    ...
    for line in f:
        ...
        instances.append(instance_and_label[0:-1])
...
def kmeans_plus_plus(instances, K):
    ...
    # select the first centre as a random number
    cent = random.choice(instances1)
    centers.append(cent)
    instances1.remove(cent)   

    #   loop to get the remaining centres
    for n in xrange(K-1):
        probs = []
        # assigning probabilities propotional to D(x)^2
        ...
        x = random.random()
        sumprob = 0.0
        # if the random number is less than equal to cumm prob draw number with that prob
        for i, prob in zip(instances1, probs):
            sumprob += prob
            if x < sumprob:
                centers.append(i)
                instances1.remove(i)
...
def assign_cluster_ids(instances, centers):
    ...
    for i in range(n):
        ...
        # above.
        cent_distance = [euclidean_squared(instances[i], x) for x in centers]

        # Find the minimum distance.
        min_distance = min(cent_distance)

        
        # Set the cluster id to be the index at which min_distance
        # is found in the list distances.
        cluster_ids[i] = cent_distance.index(min_distance)
...
def recompute_centers(instances, cluster_ids, centers):
    ...
    for i in range(K):
        ...
        sum_cluster = reduce(lambda x, y: [a + b for a,b in zip(x,y)], [instances[index] for index in one_cluster])
