# -*- coding: utf-8 -*-
"""
Created on Sun Sep 27 11:06:56 2015
...
def get_fold_indices(n, num_folds, fold_id):
    ...
    if fold_id == num_folds-1:
        ...
        train_indices = [i for i in range(0,(num_folds-1)*fold_size)]
        
    else:
       
        #train1 = range(0,0)
        y = [i for i in range(fold_id*fold_size)]
        z= [i for i in range((fold_id+1)*fold_size, n)]
        train_indices = y + z
...
def nn_classifier(point, train_data, train_labels, k, K):
    ...
    all_three_sorted = sorted(all_three, key=lambda tup: tup[0])

    
    # after sorting extract the labels from the first k
    # triples
    nearest_k_labels = [all_three_sorted[i][2] for i in range(len(all_three_sorted)) if i <k]
    ...
    freq=collections.Counter(nearest_k_labels)

    # find the labels that have maximum frequency
    
    max_freq_labels = [x for x,y in zip(freq.keys(),freq.values()) if y == max(freq.values())]

    # if there are several labels with max frequency,
    # return one of them at random
    return random.choice(max_freq_labels)
    
#nn_classifier(random.choice(instances), instances, labels, 3,10)
    ...
    for k in k_range:
        ...
        for fold_id in range(num_folds):
            ...
            for i in fold_train_indices:
                train_data1 = instances[i]  
                train_labels1 = labels[i]
                train_data.append(train_data1)
                train_label.append(train_labels1)
            def classifier(point):
               return nn_classifier(point, train_data, train_label, k, K)
            ...
            test_data = []
            test_label = []
            for i in fold_test_indices:
                test_data1 = instances[i] 
                test_labels1 = labels[i]
                test_data.append(test_data1)
                test_label.append(test_labels1)
